...
namespace Steeltoe.Security.Authentication.CloudFoundry.Wcf
{
  ...
  public class JwtAuthorizationManager : ServiceAuthorizationManager
  {
    ...
    private void SetPrincipal(OperationContext operationContext, ClaimsPrincipal principal)
    {
      ...
      // Set User Principal if wihtin HostedHttpContext
      // Options to check if Http Hosted
      // Opt1: ServiceHostingEnvironment.AspNetCompatibilityEnabled 
      // Opt2: operationContext.RequestContext.GetType().FullName.Equals("System.ServiceModel.Activation.HostedHttpContext", StringComparison.OrdinalIgnoreCase);
      // However, not necessary as the check for HttpContext should suffice
